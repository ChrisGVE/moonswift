use std::{collections::HashMap, iter, num::NonZeroUsize, rc::Rc};
...
// MoonSwift patch: removed LAYOUT_CACHE entirely to avoid any Rust TLS sections
// in the static lib (see ARCHITECTURE.md §5.4 arm64-TLS caveat).
// split_with_spacers now calls try_split directly on every invocation —
// no caching.  Performance cost is acceptable for the MoonSwift use case.
...
impl Layout {
    ...
    pub fn init_cache(_cache_size: NonZeroUsize) {
        // MoonSwift patch: no-op — cache removed to eliminate TLS (ARCHITECTURE.md §5.4).
    }
    ...
    pub fn split_with_spacers(&self, area: Rect) -> (Segments, Spacers) {
        // MoonSwift patch: no cache — call try_split directly on every invocation
        // to avoid Rust TLS in the static lib (ARCHITECTURE.md §5.4).
...
mod tests {
    ...
    #[test]
    fn cache_size() {
        // MoonSwift patch: cache removed — init_cache is a no-op, no assertions needed.
