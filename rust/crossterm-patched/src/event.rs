...
// MoonSwift patch: replaced parking_lot with std::sync — avoids Rust TLS sections
// in the static lib on arm64 macOS (ARCHITECTURE.md §5.4).
static INTERNAL_EVENT_READER: OnceLock<Mutex<InternalEventReader>> = OnceLock::new();

fn internal_event_reader() -> &'static Mutex<InternalEventReader> {
    INTERNAL_EVENT_READER.get_or_init(|| Mutex::new(InternalEventReader::default()))
}

pub(crate) fn lock_internal_event_reader() -> MutexGuard<'static, InternalEventReader> {
    internal_event_reader().lock().unwrap()
}
fn try_lock_internal_event_reader_for(
    _duration: Duration,
) -> Option<MutexGuard<'static, InternalEventReader>> {
    // MoonSwift patch: std::sync::Mutex has no timed try_lock; use non-blocking
    // try_lock instead. Sufficient for the crossterm poll path in our use case.
    internal_event_reader().try_lock().ok()
}
