...
function format.builtin_formatters.JUnit(report, file_names)
    ...
    for file_i, file_report in ipairs(report) do
        if file_report.fatal then
            table.insert(
                buf,
                ([[    <testcase name="%s" classname="%s">]]):format(
                    escape_xml(file_names[file_i]),
                    escape_xml(file_names[file_i])
                )
            )
        ...
        elseif #file_report == 0 then
            ...
            for event_i, event in ipairs(file_report) do
                table.insert(
                    buf,
                    ([[    <testcase name="%s:%d" classname="%s">]]):format(
                        escape_xml(file_names[file_i]),
                        event_i,
                        escape_xml(file_names[file_i])
                    )
                ...
                table.insert(
                    buf,
                    ([[        <failure type="%s" message="%s"/>]]):format(
...
function format.builtin_formatters.visual_studio(report, file_names)
    ...
    for i, file_report in ipairs(report) do
        if file_report.fatal then
            ...
            table.insert(
                buf,
                ("luacheck : fatal error %s: couldn't check %s: %s"):format(
                    fatal_error_codes[file_report.fatal],
                    file_names[i],
                    file_report.msg
                )
            )
